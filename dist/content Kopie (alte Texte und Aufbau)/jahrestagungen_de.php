<?php
  // content/jahrestagungen_de.php

?>

<div class="container-fluid">
  <div class="row">
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
      <h2>Jahrestagungen</h2>
      <p>
        Hier finden Sie Informationen zur nächsten Jahrestagung und einen
        Rückblick der vergangenen Jahrestagungen
      </p>
    </div>
  </div>
  <div class="row">
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
      <h3>15. Jahrestagung der DGfO</h3>
      <p>
        Die 15. Jahrestagung der DGfO findet vom 23. bis 25. März 2018 im
        <a href="https://www.potsdam.de/kategorie/naturkundemuseum-potsdam-0" target="_blank">
          Naturkundemuseum Potsdam
        </a> statt. Gleichzeitig feiern wir das
        30-jährige Bestehen der Deutschen Gesellschaft für Orthopterologie
      </p>
      <ul class='dgfo'>
         <li><a href="downloads/tagungen/2018_15_jahrestagung_dgfo_potsdam_ankuendigung.pdf" target="_blank">Flyer</a></li>
         <li><a href="downloads/tagungen/2018_15_jahrestagung_dgfo_potsdam.pdf" target="_blank">Informationen und Anmeldeformular</a> </li>
      </ul>
    </div>
  </div>
  <div class="row">
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
      <h3>Zurückliegende Jahrestagungen</h3>
    </div>
  </div>
  <!-- <div class="row">
    <div class="col-lg-2 col-md-2 col-sm-4 col-xs-4">
      <p>2018 - Potsdam</p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p></p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p></p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p></p>
    </div>
  </div> -->
  <div class="row">
    <div class="col-lg-2 col-md-2 col-sm-4 col-xs-4">
      <p>2016 - Trier</p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
        <a href="downloads/tagungen/2016_14_jahrestagung_dgfo_trier.pdf" target="_blank">
          Programm
          &nbsp;<i class="fa fa-file-pdf-o" aria-hidden="true"></i>
        </a>
      </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
        <a href="downloads/tagungen/2016_14_jahrestagung_dgfo_trier.pdf" target="_blank">
          Abstracts
          &nbsp;<i class="fa fa-file-pdf-o" aria-hidden="true"></i>
        </a>
      </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p></p>
    </div>
  </div>
  <div class="row">
    <div class="col-lg-2 col-md-2 col-sm-4 col-xs-4">
      <p>2012 - Bernburg</p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
        <a href="downloads/tagungen/2012_12_jahrestagung_dgfo_bernburg.pdf" target="_blank">
          Programm
          &nbsp;<i class="fa fa-file-pdf-o" aria-hidden="true"></i>
        </a>
      </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
    </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>Fotos</p>
    </div>
  </div>
  <div class="row">
    <div class="col-lg-2 col-md-2 col-sm-4 col-xs-4">
      <p>2010 - Mainz</p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
        <a href="downloads/tagungen/2012_11_jahrestagung_dgfo_mainz.pdf" target="_blank">
          Programm
          &nbsp;<i class="fa fa-file-pdf-o" aria-hidden="true"></i>
        </a>
      </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
    </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>Fotos</p>
    </div>
  </div>
  <div class="row">
    <div class="col-lg-2 col-md-2 col-sm-4 col-xs-4">
      <p>2008 - Bonn</p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
        <a href="downloads/tagungen/2008_10_jahrestagung_dgfo_bonn.pdf" target="_blank">
          Programm
          &nbsp;<i class="fa fa-file-pdf-o" aria-hidden="true"></i>
        </a>
      </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
    </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>Fotos</p>
    </div>
  </div>
  <div class="row">
    <div class="col-lg-2 col-md-2 col-sm-4 col-xs-4">
      <p>2006 - Augsburg</p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
        <a href="downloads/tagungen/2006_09_jahrestagung_dgfo_augsburg.pdf" target="_blank">
          Programm
          &nbsp;<i class="fa fa-file-pdf-o" aria-hidden="true"></i>
        </a>
      </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
        <a href="downloads/tagungen/2006_09_jahrestagung_dgfo_augsburg.pdf" target="_blank">
          Abstracts
          &nbsp;<i class="fa fa-file-pdf-o" aria-hidden="true"></i>
        </a>
      </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>Fotos</p>
    </div>
  </div>
  <div class="row">
    <div class="col-lg-2 col-md-2 col-sm-4 col-xs-4">
      <p>2004 - Osnabrück</p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
        <a href="downloads/tagungen/2004_08_jahrestagung_dgfo_osnabrueck.pdf" target="_blank">
          Programm
          &nbsp;<i class="fa fa-file-pdf-o" aria-hidden="true"></i>
        </a>
      </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
        <a href="downloads/tagungen/2004_08_jahrestagung_dgfo_osnabrueck.pdf" target="_blank">
          Abstracts
          &nbsp;<i class="fa fa-file-pdf-o" aria-hidden="true"></i>
        </a>
      </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>Fotos</p>
    </div>
  </div>
  <div class="row">
    <div class="col-lg-2 col-md-2 col-sm-4 col-xs-4">
      <p>2002 - Münster</p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
        <a href="downloads/tagungen/2002_07_jahrestagung_dgfo_muenster.pdf" target="_blank">
          Programm
          &nbsp;<i class="fa fa-file-pdf-o" aria-hidden="true"></i>
        </a>
      </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
        <a href="downloads/tagungen/2002_07_jahrestagung_dgfo_muenster_abstracts.pdf" target="_blank">
          Abstracts
          &nbsp;<i class="fa fa-file-pdf-o" aria-hidden="true"></i>
        </a>
      </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>Fotos</p>
    </div>
  </div>
  <div class="row">
    <div class="col-lg-2 col-md-2 col-sm-4 col-xs-4">
      <p>2000 - Potsdam</p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
        <a href="downloads/tagungen/2000_06_jahrestagung_dgfo_potsdam.pdf" target="_blank">
          Programm
          &nbsp;<i class="fa fa-file-pdf-o" aria-hidden="true"></i>
        </a>
      </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
      </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>Fotos</p>
    </div>
  </div>
  <div class="row">
    <div class="col-lg-2 col-md-2 col-sm-4 col-xs-4">
      <p>1998 - Bremen</p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
        <a href="downloads/tagungen/1998_05_jahrestagung_dgfo_bremen.pdf" target="_blank">
          Programm
          &nbsp;<i class="fa fa-file-pdf-o" aria-hidden="true"></i>
        </a>
      </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
        <a href="downloads/tagungen/1998_05_jahrestagung_dgfo_bremen.pdf" target="_blank">
          Abstracts
          &nbsp;<i class="fa fa-file-pdf-o" aria-hidden="true"></i>
        </a>
      </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p></p>
    </div>
  </div>
  <div class="row">
    <div class="col-lg-2 col-md-2 col-sm-4 col-xs-4">
      <p>1996 - Marbug</p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
        <a href="downloads/tagungen/1996_04_jahrestagung_dgfo_marburg.pdf" target="_blank">
          Programm
          &nbsp;<i class="fa fa-file-pdf-o" aria-hidden="true"></i>
        </a>
      </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
      </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
      </p>
    </div>
  </div>
  <div class="row">
    <div class="col-lg-2 col-md-2 col-sm-4 col-xs-4">
      <p>1994 - Jena</p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
        <a href="downloads/tagungen/1994_03_jahrestagung_dgfo_jena.pdf" target="_blank">
          Programm
          &nbsp;<i class="fa fa-file-pdf-o" aria-hidden="true"></i>
        </a>
      </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
      </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
      </p>
    </div>
  </div>
  <div class="row">
    <div class="col-lg-2 col-md-2 col-sm-4 col-xs-4">
      <p>1992 - Bonn</p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
        <a href="downloads/tagungen/1992_02_jahrestagung_dgfo_bonn.pdf" target="_blank">
          Programm
          &nbsp;<i class="fa fa-file-pdf-o" aria-hidden="true"></i>
        </a>
      </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
        <a href="downloads/tagungen/1992_02_jahrestagung_dgfo_bonn.pdf" target="_blank">
          Abstracts
          &nbsp;<i class="fa fa-file-pdf-o" aria-hidden="true"></i>
        </a>
      </p>    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
      </p>
    </div>
  </div>
  <div class="row">
    <div class="col-lg-2 col-md-2 col-sm-4 col-xs-4">
      <p>1990 - Erlangen</p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
        <a href="downloads/tagungen/1990_01_jahrestagung_dgfo_erlangen" target="_blank">
          Programm
          &nbsp;<i class="fa fa-file-pdf-o" aria-hidden="true"></i>
        </a>
      </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
      </p>
    </div>
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
      <p>
      </p>
    </div>
  </div>
</div>
